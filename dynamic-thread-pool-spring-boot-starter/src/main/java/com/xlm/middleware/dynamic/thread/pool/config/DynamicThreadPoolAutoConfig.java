...
import com.xlm.middleware.dynamic.thread.pool.domain.model.vo.RegistryEnumVO;
import com.xlm.middleware.dynamic.thread.pool.registry.IRegistry;
import com.xlm.middleware.dynamic.thread.pool.registry.redis.RedisRegistry;
import com.xlm.middleware.dynamic.thread.pool.trigger.job.ThreadPoolDataReportJob;
import com.xlm.middleware.dynamic.thread.pool.trigger.listener.ThreadPoolConfigAdjustListener;
 ...
 * 初始化：
 * RedissonClient
 * DynamicThreadPoolService（ApplicationContext, Map<String,ThreadPoolExecutor）查看和更新线程池数据
 * IRegistry(RedissonClient) 注册中心、操作redis
 * ThreadPoolDataReportJob（DynamicThreadPoolService，IRegistry）从service查数据，更新redis
 * RTopic(RedissonClient，ThreadPoolConfigAdjustListener) pub-sub模型
 * ThreadPoolConfigAdjustListener 发布订阅、redis变化后，会通知Serivce去更新线程池数据
 */
@Configuration
